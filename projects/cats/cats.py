...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    choices = [i for i in paragraphs if select(i)]
    if len(choices) > k:
        return choices[k]
    else:
        return ""
...
def about(subject):
    ...
    def select(paragraph):
        ...
        p_list = [lower(remove_punctuation(word)) for word in p_list]
        for i in subject:
            if i in p_list:
                return True
        return False
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    ret = [[] for x in player_indices]
    for wi in word_indices:
        fastestPi = 0
        for pi in player_indices:
            if time(match, pi, wi) < time(match, fastestPi, wi):
                fastestPi = pi
        ret[fastestPi].append(get_word(match, wi))
    return ret
    # END PROBLEM 10
