...
def tail_points(opponent_score):
    ...
    assert opponent_score >= 0, "Must at least zero point."
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        while score1 < goal:
            if strategy(score0, score1) != NUM_ROLLS:
                return False
            score1 += 1 
        score0 += 1
        score1 = 0
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def average_original_fn(*args):
        sum, i = 0, 0
        while i < total_samples:
            sum += original_function(*args)
            i += 1
        return sum / total_samples
    return average_original_fn
    # END PROBLEM 8
